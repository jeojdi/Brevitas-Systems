...
if [[ -z "${DATABASE_URL:-}" ]]; then
  echo 'DATABASE_URL is required for the shared billing recovery limiter test.' >&2
  exit 2
fi
...
case "${server_address}" in
  127.0.0.1|::1) ;;
  *)
    echo 'Shared billing recovery limiter tests require loopback PostgreSQL.' >&2
    exit 2
    ;;
esac
...
# Independent sessions model separate Vercel instances. The database must
# admit exactly five attempts for one authenticated actor/company identity.
worker_pids=()
for worker_index in $(seq 1 12); do
  psql "${DATABASE_URL}" --no-psqlrc --set ON_ERROR_STOP=1 --tuples-only --no-align \
    --command "select public.consume_billing_recovery_attempt(
...
if [[ "${accepted_count}" -ne 5 || "${limited_count}" -ne 7 ]]; then
  echo "Cross-instance billing recovery limiter admitted ${accepted_count} and denied ${limited_count}; expected 5/7." >&2
  exit 1
fi
