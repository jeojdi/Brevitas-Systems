...
6. **Holdout disclosure: language DRAFTED, awaiting James's approval.** The ~5% (org, prefix) control arm is built and off by default (`BREVITAS_WARM_HOLDOUT_PCT=0`); it will not be switched on for any org until the disclosure clause below is approved and in the terms/DPA template. Draft follows.

### Holdout disclosure draft (awaiting approval)
