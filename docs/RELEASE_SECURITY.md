...
Repository readiness is enforced by three GitHub Actions workflows. All third-party actions
...
2. applies the exact 13-file upgrade suffix `202607170001` through `202607170013`, including a
...
6. verifies service-role-only RPC permissions, tenant isolation, equal-sort cursor stability,
   ...
   server-owned active-company persistence/fallback, and the receipt-accounting
   fields/permissions/billing-trigger contract;
7. applies migrations 010–013 twice, including a simulated quarantined pre-constraint receipt
   whose ciphertext must be erased before the named constraint validates, then
...
8. resets only the verified loopback database, applies all 25 forward Supabase files as an
   isolated fresh install, reapplies migrations 010–013, and reruns the forward-contract
   assertions.
...
`scripts/ci/migration-frozen-checksums.txt` pins the final bodies of migrations 007 and 009–012,
...
compressor), billing authentication, and the manual-recovery authentication/parser path. Tenant A
