"""Thin adapter for retrieval-based context selection.
...
returned (never silently thinned). Retrieval can still omit evidence, so it remains
experimental and unverified until a paired workload passes. Savings are measured with the
real tokenizer, never a proxy. The embedding model is lazy-loaded once per process.
"""
...
_BROAD_QUERY = re.compile(
    r"\b(summar(?:ize|ise|y)|all|every|entire|whole|comprehensive|overview|"
    r"rank\s+(?:all|each)|compare\s+(?:all|each))\b",
    re.IGNORECASE,
)
...
def retrieval_select(task: str, prior_context: List[str], k: int = 5,
    ...
    """Select prior-context chunks relevant to ``task`` and fail safe to full context.

    Returns selected_context, baseline/optimized tokens (real tokenizer), savings_pct,
    and fallback metadata. Never raises; on any error returns the full context. Retrieval
    reduces evidence and therefore is not inherently lossless; quality-first mode combines
    dense and sparse rankings and protects explicit second-hop links, but remains experimental.
    ...
    Args:
        ...
        use_adaptive: if True, use quality-first hybrid RRF + a bounded bridge hop;
                      if False, use fixed-k DPR (legacy/backwards-compatible path)
    """
    ...
    if not prior_context:
        return {"selected_context": [], "baseline_tokens": 0, "optimized_tokens": 0,
                ...
                "method": "none", "quality_status": "no_context"}
    if not (task or "").strip():
        return {"selected_context": list(prior_context), "baseline_tokens": baseline,
                "optimized_tokens": baseline, "savings_pct": 0.0,
                "fallback_applied": True, "reason": "empty_query",
                "method": "full_context", "quality_status": "full_context_fallback"}
    if _BROAD_QUERY.search(task):
        return {"selected_context": list(prior_context), "baseline_tokens": baseline,
                "optimized_tokens": baseline, "savings_pct": 0.0,
                "fallback_applied": True, "reason": "broad_query",
                "method": "full_context", "quality_status": "full_context_fallback"}
    ...
    try:
        ...
        if use_adaptive:
            cfg = QualityFirstRetrievalConfig(
                max_k=max(10, k),
                min_k=k,
                min_top_score=min_top_score,
                fallback_to_full=True,
            )
            chosen, meta = fetch_quality_first(r, task, prior_context, cfg)
        else:
            # Legacy: fixed-k DPR
    ...
    # Taking evidence risk for a negligible reduction has negative expected value.
    if not meta.get("fallback_applied", False) and optimized >= baseline * 0.98:
        chosen = list(prior_context)
        optimized = baseline
        meta = {"fallback_applied": True, "reason": "negligible_savings"}
    savings = round(max(0.0, (1 - optimized / max(1, baseline)) * 100), 2)
    quality_status = (
        "full_context_fallback" if meta.get("fallback_applied", False)
    ...
    return {"selected_context": chosen, "baseline_tokens": baseline,
            ...
            "method": meta.get("method", "fixed_dense"),
            "quality_status": quality_status,
            "bridge_expansions": meta.get("bridge_expansions", 0),
            "dense_sparse_top_overlap": meta.get("dense_sparse_top_overlap")}
